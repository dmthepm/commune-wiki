...
- `pnpm test` → 34 pass. `pnpm build` → green, prints `41 total backlinks across 11 entries` and `PASS: 1 standalone page indexed…`; `public/backlinks.json` is committed and byte-identical after a build. 32 `Broken link` warnings on the engine's own template content today.
...
- **[M3] The success sentence is not falsifiable on the numbers that matter.** It names `15 pages, 41 backlinks across 11 entries` for the build but gives no expected result for the two new commands. Measured baselines exist: engine `check` must report 32 `broken-link` findings and 0 `duplicate-name`/`ambiguous-target`; devon-wiki `graph query` must return 78 entries with 372 resolved edges and 0 broken. Without these, "runs on the live corpus" is satisfied by any output. — **Fix:** put the numbers in the sentence (v2 below) and in the PR receipt.
...
```json
{ "schema": 1, "root": "/abs/project",
  "summary": { "entries": 11, "edges": 41, "errors": 0, "warnings": 32,
               "byRule": { "broken-link": 32, "ambiguous-target": 0, "duplicate-name": 0, "noncanonical-title": 0 } },
...
4. **`check`.** Core `checkEntries`; canonical-title rule moved out of `scripts/test-search-index.mjs`, which now imports it and keeps `FAIL`/exit 1. *Verify:* engine `commune check --json` reports `warnings: 32, errors: 0`; fixture reports one of each rule; `pnpm build` still prints `PASS`.
